			...
			<table class="table"
				...
				<c:forEach items="${pageInfo.list}" var="user">
					<tr>
						...
						<td>${user.uSex}</td>
						<td>${user.uEmail}</td>
						<td><a href="passcheck.action?username=${user.uUsername}">通过审核</a>||<a href="deletecheck.action?username=${user.uUsername}">删除</a></td>
					</tr>
				</c:forEach>
			</table>
			<!--显示分页信息-->
			<div class="row" style="text-align: center;">
				...
				<div class="col-md-6">
					<nav aria-label="Page navigation">
						<ul class="pagination">
							<li><a
								href="${pageContext.request.contextPath}/checkmember.action?teamName=${teamName}&&pn=1">首页</a></li>

							<!--上一页-->
							<li><c:if test="${pageInfo.hasPreviousPage}">
									<a
										href="${pageContext.request.contextPath}/checkmember.action?teamName=${teamName}&&pn=${pageInfo.pageNum-1}"
							...
							<c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
								...
								<c:if test="${page_num != pageInfo.pageNum}">
									<li><a
										href="${pageContext.request.contextPath}/checkmember.action?teamName=${teamName}&&pn=${page_num}">${page_num}</a></li>
								</c:if>
							</c:forEach>
							...
							<li><a
								href="${pageContext.request.contextPath}/checkmember.action?teamName=${teamName}&&pn=${pageInfo.pages}">尾页</a></li>
						</ul>
					</nav>
				</div>
			</div>
