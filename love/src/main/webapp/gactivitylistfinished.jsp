		...
		<div class="wrap" style="padding-top: 40px">
			<c:forEach items="${pageInfo.list}" var="activity" varStatus="s">
				<div class="newsunit" style="height: 230px">
					<a href="showActivity.action?activityid=${activity.id}"> <c:forEach items="${teams}" var="team" begin="${s.index}" end="${s.index}">
							<c:if
							test="${empty team.teamImage}">
						...
						<div class="fl news-text" >
							<h2>${activity.aActivity} <span style="color: #919191">${activity.aOn}</span></h2>
		...
		<div class="row" style="text-align: center;">
			...
			<div class="col-md-6">
				<nav aria-label="Page navigation">
					<ul class="pagination">
						<li><a href="${pageContext.request.contextPath}/showAllActivityFinished.action?teamName=${user.uTeamName}&&pn=1">首页</a></li>

						<!--上一页-->
						<li><c:if test="${pageInfo.hasPreviousPage}">
								<a
									href="${pageContext.request.contextPath}/showAllActivityFinished.action?teamName=${user.uTeamName}&&pn=${pageInfo.pageNum-1}"
						...
						<c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
							...
							<c:if test="${page_num != pageInfo.pageNum}">
								<li><a
									href="${pageContext.request.contextPath}/showAllActivityFinished.action?teamName=${user.uTeamName}&&pn=${page_num}">${page_num}</a></li>
							</c:if>
						</c:forEach>
						...
						<li><a
							href="${pageContext.request.contextPath}/showAllActivityFinished.action?teamName=${user.uTeamName}&&pn=${pageInfo.pages}">尾页</a></li>
					</ul>
				</nav>
			</div>
		</div>
