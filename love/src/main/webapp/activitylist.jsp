				...
				<div class="newsunit" style="height: 230px">
					...
					<c:forEach items="${teams}" var="team" begin="${s.index}" end="${s.index}">
						...
						<div class="fl news-text">
							<h2>${activity.aActivity }<span style="color: #fa8072">${activity.aOn}</span>
							</h2>
		...
		<div class="row" style="text-align: center;">
			...
			<div class="col-md-6">
				<nav aria-label="Page navigation">
					<ul class="pagination">
						<li><a
							href="${pageContext.request.contextPath}/showAllActivity.action?pn=1">首页</a></li>

						<!--上一页-->
						<li><c:if test="${pageInfo.hasPreviousPage}">
								<a
									href="${pageContext.request.contextPath}/showAllActivity.action?Spn=${pageInfo.pageNum-1}"
						...
						<c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
							...
							<c:if test="${page_num != pageInfo.pageNum}">
								<li><a
									href="${pageContext.request.contextPath}/showAllActivity.action?pn=${page_num}">${page_num}</a></li>
							</c:if>
						</c:forEach>
						...
						<li><c:if test="${pageInfo.hasNextPage}">
								<a
									href="${pageContext.request.contextPath}/showAllActivity.action?pn=${pageInfo.pageNum+1}"
									aria-label="Next"> <span aria-hidden="true">»</span>
								</a>
							</c:if></li>

						<li><a
							href="${pageContext.request.contextPath}/showAllActivity.action?pn=${pageInfo.pages}">尾页</a></li>
					</ul>
				</nav>
			</div>
		</div>
